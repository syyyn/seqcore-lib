Contributing to Bitcore SEQ
=======

We're working hard to make *seqcore* the most powerful JavaScript library for working with sequence. Our goal is to have *seqcore* be a library that can be used by anyone interested in sequence, and to level expertise differences with great design and documentation.
...
These are some global design goals in seqcore that any change must adhere.
...
When possible, seqcore objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of Bitcore SEQ methods. For example, calling `JSON.stringify` on a Bitcore SEQ object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'seqcore' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the seqcore object.

DO:
```javascript
var seqcore = require('../');
var PublicKey = seqcore.PublicKey;
```
DON'T:
```javascript
...
```sh
git remote add sequence-project git@github.com:sequence-project/seqcore.git
git pull --rebase sequence-project master
```
...
```
Finally go to [github.com/sequence-project/seqcore](https://github.com/sequence-project/seqcore) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of seqcore.

If you have any questions feel free to post them to
[github.com/sequence-project/seqcore/issues](https://github.com/sequence-project/seqcore/issues).
