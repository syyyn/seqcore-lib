...
async.series([
  function(next) {
    ...
    for (var i = 0; i < 100; i++) {
      ...
      // uint64le
      var br = new seqcore.encoding.BufferWriter();
      var num = Math.round(Math.random() * 10000000000000);
      br.writeUInt64LEBN(new seqcore.crypto.BN(num));
      buffers.push(br.toBuffer());

      // hashes
      var data = seqcore.crypto.Hash.sha256sha256(new Buffer(32));
    ...
    function readUInt64LEBN() {
      ...
      var buf = buffers[c];
      var br = new seqcore.encoding.BufferReader(buf);
  ...
  function(next) {
    ...
    function seqcoreTest() {
      block1 = seqcore.Block.fromString(blockData);
    }
    ...
    var suite = new benchmark.Suite();
    suite.add('seqcore', seqcoreTest, {maxTime: maxTime});
